...
const Search = () => {
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    if (!searchQuery.trim()) return; // ✅ CHANGE: Prevent empty search
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to fetch books'); // ✅ CHANGE: Friendly error message
      setResults([]);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center pt-12 mt-20 dark:bg-slate-800 dark:text-white">
      <div className="w-full max-w-xl mx-auto text-center">
        ...
        <form onSubmit={handleSearch} className="flex flex-col items-center mb-10">
          <input
            type="text"
            value={searchQuery}
